...
describe('NMEAGenerator - Time Encoding with options', () => {
    ...
    test('generateGPRMC uses pos.timestamp if options.timestamp missing', () => {
        const p = { ...pos, timestamp: fixedTimestamp };
        ...
        test('generateGPRMC uses current time if no timestamps', () => {
            ...
            // Now compare with before/after window
            expect(encodedDate.getTime()).toBeGreaterThanOrEqual(before.getTime() - 500);
            expect(encodedDate.getTime()).toBeLessThanOrEqual(after.getTime() + 500);
        });
